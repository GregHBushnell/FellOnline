...
{
	public class FAbilityObject : MonoBehaviour
	{
		internal int ContainerID;
		internal int ID;
		public FAbility Ability;
		...
		void Update()
		{
			...
			if (Ability != null)
			{
				foreach (FMoveEvent moveEvent in Ability.MoveEvents.Values)
				{
					// invoke
		...
		void OnCollisionEnter(Collision other)
		{
			...
			if (Ability != null)
			{
				foreach (FHitEvent hitEvent in Ability.HitEvents.Values)
				{
					FTargetInfo targetInfo = new FTargetInfo()
					{
		...
		/// </summary>
		public static bool TrySpawn(FAbility ability, Character caster, FAbilityController controller, Transform abilitySpawner, FTargetInfo targetInfo)
		{
			FAbilityTemplate template = ability.Template;
			...
			{
				ability.Objects = new Dictionary<int, Dictionary<int, FAbilityObject>>();
			}

			Dictionary<int, FAbilityObject> abilityObjects = new Dictionary<int, FAbilityObject>();
			...
			{
				foreach (FSpawnEvent spawnEvent in ability.PreSpawnEvents.Values)
				{
			...
			{
				foreach (FSpawnEvent spawnEvent in ability.SpawnEvents.Values)
				{
					spawnEvent.Invoke(caster, targetInfo, abilityObject, ref id, abilityObjects);
				}
			}
			...
			// finalize
			foreach (FAbilityObject obj in abilityObjects.Values)
			{

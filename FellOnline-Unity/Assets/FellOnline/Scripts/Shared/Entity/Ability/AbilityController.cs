			...
			else if (IsActivating)
			{
				...
				if (remainingTime > 0.0f)
				{
					...
					if (heldKey != KeyCode.None)
					{
						...
						{
							// get target info
							TargetInfo targetInfo = t.UpdateTarget(Character.Transform.position,
																					Character.transform.rotation * Vector3.forward,
				...
				{
					// get target info
					TargetInfo targetInfo = tc.UpdateTarget(Character.Transform.position,
																					Character.transform.rotation * Vector3.forward,
																					currentAbility.Range);

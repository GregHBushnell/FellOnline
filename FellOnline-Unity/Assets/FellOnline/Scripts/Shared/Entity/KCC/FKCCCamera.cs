...
{
	public class FKCCCamera : MonoBehaviour
	{
		[Header("Framing")]
		...
		public float DistanceMovementSharpness = 1f;
...
//, Vector3 rotationInput
		public void UpdateWithInput(float deltaTime, float zoomInput)
		{
			if (FollowTransform)
			{
				...
				_currentDistance = Mathf.Lerp(_currentDistance, TargetDistance, 1 - Mathf.Exp(-DistanceMovementSharpness * deltaTime));
				Camera.orthographicSize = _currentDistance;
				 Vector3 desiredPosition = _currentFollowPosition + offset;
				

            	Vector3 smoothedPosition = Vector3.Lerp(Transform.position, desiredPosition, DistanceMovementSharpness* deltaTime);

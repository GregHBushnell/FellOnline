...
{
	public abstract class FItemContainer : NetworkBehaviour
	{
		public readonly List<FItem> Items = new List<FItem>();

		public event Action<FItemContainer, FItem, int> OnSlotUpdated;
		...
		/// </summary>
		public bool TryGetItem(int slot, out FItem item)
		{
			if (IsValidSlot(slot))
			{
				item = Items[slot];
		...
		/// </summary>
		public void AddSlots(List<FItem> items, int amount)
		{
			if (items != null)
			{
		...
		public bool CanAddItem(FItem item)
		{
			if (!CanManipulate())
			{
				return false;
		...
		/// </summary>
		public bool TryAddItem(FItem item, out List<FItem> modifiedItems)
		{
			modifiedItems = new List<FItem>();
		...
		/// </summary>
		public bool SetItemSlot(FItem item, int slot)
		{
			if (!IsValidSlot(slot))
			{
		...
		{
			return SwapItemSlots(from, to, out FItem fromItem, out FItem toItem);
		}

		public bool SwapItemSlots(int from, int to, out FItem fromItem, out FItem toItem)
		{
			if (!CanManipulate() ||
				from < 0 ||
				to < 0 ||
		...
		/// </summary>
		public FItem RemoveItem(int slot)
		{
			if (!CanManipulate() ||
				!IsValidSlot(slot))
			{

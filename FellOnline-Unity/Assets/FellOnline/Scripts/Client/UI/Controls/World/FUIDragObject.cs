...
{
	public class FUIDragObject : FUIControl
	{
		...
		void Update()
		{
			if (Visible)
			{
				...
				if (Input.GetMouseButtonDown(0) && !FUIManager.ControlHasFocus())
				{
					// we can drop items on the ground from inventory
					if (Type == FReferenceButtonType.Inventory)
					{
		...
		public void SetReference(Sprite icon, long referenceID, FReferenceButtonType type)
		{
			Icon.sprite = icon;

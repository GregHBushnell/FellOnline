...
{
	public class FClientLoginAuthenticator : Authenticator
	{
		private string username = "";
		private string password = "";
		private bool register;
		private FClientSrpData SrpData;
		...
		/// </summary>
		public event Action<FClientAuthenticationResult> OnClientAuthenticationResult;
...
#pragma warning restore CS0067
		...
		private void ClientManager_OnClientConnectionState(ClientConnectionStateArgs args)
		{
			...
			* example the client tries to authenticate soon as they connect. */
			...
			SrpData = new FClientSrpData(SrpParameters.Create2048<SHA512>());

			// register a new account?
			if (register)
			{
				SrpData.GetSaltAndVerifier(username, password, out string salt, out string verifier);
				FCreateAccountBroadcast msg = new FCreateAccountBroadcast()
				{

...
{
	public class FCharacterAbilityService
	{
		...
		/// </summary>
		public static void UpdateOrAdd(NpgsqlDbContext dbContext, long characterID, FAbility ability)
		{
			if (characterID == 0)
			{
				return;
		...
		public static void Save(NpgsqlDbContext dbContext, Character character)
		{
			...
			foreach (KeyValuePair<long, FAbility> pair in character.AbilityController.KnownAbilities)
			{
				if (pair.Key < 0)
				{
					continue;
		...
		public static void Load(NpgsqlDbContext dbContext, Character character)
		{
			...
			{
				FAbilityTemplate template = FAbilityTemplate.Get<FAbilityTemplate>(dbAbility.TemplateID);
				if (template != null)
				{
					character.AbilityController.LearnAbility(new FAbility(dbAbility.ID, template, dbAbility.AbilityEvents));
				}
			};
		}
	}

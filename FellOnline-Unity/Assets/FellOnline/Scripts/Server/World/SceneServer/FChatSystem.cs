...
namespace FellOnline.Server
{
	...
	/// Server chat system.
	/// </summary>
	public class FChatSystem : FServerBehaviour, IChatHelper
	{
		...
		private List<ChatEntity> FetchChatMessages()
		{
			...
			List<ChatEntity> messages = FChatService.Fetch(dbContext, lastFetchTime, lastFetchPosition, MessageFetchCount, Server.SceneServerSystem.ID);
			if (messages != null)
			{
		...
		private void ProcessNewChatMessage(NetworkConnection conn, Character sender, ChatBroadcast msg)
		{
			...
			msg.text = FChatHelper.Sanitize(msg.text);

			string cmd = FChatHelper.GetCommandAndTrim(ref msg.text);

			// direct commands are handled differently
			if (FChatHelper.TryParseDirectCommand(cmd, sender, msg))
			{
				return;
			}
			...
			ChatCommand command = FChatHelper.ParseChatCommand(cmd, ref msg.channel);
			if (command != null)
			{
				...
				if (command.Invoke(sender, msg))
				{
					...
					using var dbContext = Server.NpgsqlDbContextFactory.CreateDbContext();
					FChatService.Save(dbContext, sender.ID.Value, sender.WorldServerID, Server.SceneServerSystem.ID, msg.channel, msg.text);
				}
			}
		}
		...
		{
			// get the world ID
			string wid = FChatHelper.GetWordAndTrimmed(msg.text, out string trimmed);
		...
		public bool OnPartyChat(Character sender, ChatBroadcast msg)
		{
			...
			// get the party ID
			string gid = FChatHelper.GetWordAndTrimmed(msg.text, out string trimmed);
			...
			using var dbContext = Server.NpgsqlDbContextFactory.CreateDbContext();
			List<CharacterPartyEntity> dbMembers = FCharacterPartyService.Members(dbContext, partyID);
		...
		public bool OnGuildChat(Character sender, ChatBroadcast msg)
		{
			...
			// get all the member data so we can broadcast
			using var dbContext = Server.NpgsqlDbContextFactory.CreateDbContext();
			List<CharacterGuildEntity> dbMembers = FCharacterGuildService.Members(dbContext, guildID);
		...
		{
			// get the target
			string targetName = FChatHelper.GetWordAndTrimmed(msg.text, out string trimmed);
			...
			{
				using var dbContext = Server.NpgsqlDbContextFactory.CreateDbContext();
				targetID = FCharacterService.GetIdByName(dbContext, targetName);
				online = FCharacterService.ExistsAndOnline(dbContext, targetID);
			}
			...
			if (sender != null)
			{
				...
				if (msg.senderID == targetID)
				{
					sender.Owner.Broadcast(new ChatBroadcast()
					{
						channel = msg.channel,
						senderID = msg.senderID,
						text = FChatHelper.ERROR_MESSAGE_SELF + " ",
				...
				else if (!online)
				{
					...
					sender.Owner.Broadcast(new ChatBroadcast()
					{
						channel = msg.channel,
						senderID = msg.senderID,
						text = FChatHelper.ERROR_TARGET_OFFLINE + " " + targetName,
					}, true, Channel.Reliable);
					return false;
				}
				else if (targetID > 0)
				{
					sender.Owner.Broadcast(new ChatBroadcast()
					{
						channel = msg.channel,
						senderID = targetID,
						text = FChatHelper.RELAYED + " " + trimmed,
					}, true, Channel.Reliable);
				}
			}

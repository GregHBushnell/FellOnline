...
namespace FellOnline.Server
{
	...
	public class FCharacterSystem : FServerBehaviour
	{
		...
		void LateUpdate()
		{
			if (serverState == LocalConnectionState.Started)
			{
				if(nextOutOfBoundsCheck < 0)
				{
					...
					{
						if(Server.SceneServerSystem.WorldSceneDetailsCache.Scenes.TryGetValue(character.SceneName.Value, out FWorldSceneDetails details))
						{
				...
				if (nextSave < 0)
				{
					...
					if (CharactersByID.Count > 0)
					{
						...
						FCharacterService.Save(dbContext, new List<Character>(CharactersByID.Values));
					}
				}
		...
		private void OnApplicationQuit()
		{
			if (Server != null && Server.NpgsqlDbContextFactory != null)
			{
				...
				using var dbContext = Server.NpgsqlDbContextFactory.CreateDbContext();
				FCharacterService.Save(dbContext, new List<Character>(CharactersByID.Values), false);
			}
		}
		...
		{
			if (!authenticated || !FAccountManager.GetAccountNameByConnection(conn, out string accountName))
			{
			...
			if (FCharacterService.TryGetSelectedDetails(dbContext, accountName, out long selectedCharacterID))
			{
				...
				if (FCharacterService.TryGet(dbContext, selectedCharacterID, Server.NetworkManager, out Character character))
				{
					// check if the scene is valid, loaded, and cached properly
					if (Server.SceneServerSystem.TryGetSceneInstanceDetails(character.WorldServerID, character.SceneName.Value, character.SceneHandle, out FSceneInstanceDetails instance) &&
					...
					else
					{
						...
						WorldServerEntity worldServer = FWorldServerService.GetServer(dbContext, character.WorldServerID);
						if (worldServer != null)
						{
		...
		private void SceneManager_OnClientLoadedStartScenes(NetworkConnection conn, bool asServer)
		{
			if (WaitingSceneLoadCharacters.TryGetValue(conn, out Character character))
			{
				...
				{
					using var dbContext = Server.NpgsqlDbContextFactory.CreateDbContext();
					FCharacterService.SetOnline(dbContext, accountName, character.CharacterName);
				}
		...
		public void SendAllCharacterData(Character character)
		{
			...
			if (character.AbilityController != null)
			{
				...
				if (character.AbilityController.KnownAbilities != null)
				{
					...
					foreach (FAbility ability in character.AbilityController.KnownAbilities.Values)
					{
			...
			if (character.AchievementController != null)
			{
				...
				foreach (FAchievement achievement in character.AchievementController.Achievements.Values)
				{
			...
			if (character.GuildController != null && character.GuildController.ID.Value > 0)
			{
				...
				List<CharacterGuildEntity> dbMembers = FCharacterGuildService.Members(dbContext, character.GuildController.ID.Value);
			...
			if (character.PartyController != null && character.PartyController.ID > 0)
			{
				...
				List<CharacterPartyEntity> dbMembers = FCharacterPartyService.Members(dbContext, character.PartyController.ID);
			...
			if (character.FriendController != null)
			{
				...
				{
					bool status = FCharacterService.ExistsAndOnline(dbContext, friendID);
			...
			if (character.EquipmentController != null)
			{
				...
				foreach (FItem item in character.EquipmentController.Items)
				{
					// just in case..
					if (item == null)
					{
			...
			#region Appearance
			if (character.AppearanceController != null)
			{
				AppearanceUpdateBroadcast appearanceBroadcast = new AppearanceUpdateBroadcast();
				CharacterAppearanceDetails appearanceDetails = character.AppearanceController._appearanceDetails.Value;
		...
		private RespawnPosition GetRandomRespawnPoint(FRespawnPositionDictionary respawnPoints)
		{
